...
protected:
    ...
    void drawRightLane(QPainter *painter);
private:

    unsigned int m_room_id_;
    ...
    // 0:front 1:left 2：right 3:inner
    int m_viewMode;                //

    SignalRssi::Position m_pannel_position;

    int m_left_camera_geometry_x{10};
    int m_left_camera_geometry_y{600};
    int m_left_camera_geometry_w{512};
    int m_left_camera_geometry_h{288};

    int m_right_camera_geometry_x{10};
    int m_right_camera_geometry_y{600};
    int m_right_camera_geometry_w{512};
    int m_right_camera_geometry_h{288};
...
public:
    ...
    int getRightCameraGeometryX()       const;
    int getRightCameraGeometryY()       const;
    int getRightCameraGeometryWidth()   const;
    int getRightCameraGeometryHeight()  const;

    int getLeftCameraGeometryX()    const;
    int getLeftCameraGeometryY()    const;
    int getLeftCameraGeometryWidth()    const;
    int getLeftCameraGeometryHeight()    const;
...
public slots:
    ...
    void updateImage(int position, double timestamp, const QImage &image);
//    void updateLeftCameraImage(double timestamp, const QImage &image);
//    void updateRightCameraImage(double timestamp, const QImage &image);
    //校验设备
    void checkVideo();
    //处理按钮单击
    void btnClicked();
...
public slots:
    ...
    void setLeftCameraGeometry(int x, int y, int width, int height);
    void setRightCameraGeometry(int x, int y, int width, int height);
