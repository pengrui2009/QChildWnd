...
protected:
    ...
    void detect_edge(QMouseEvent *event);
...
//    liteav::trtc::TctcSendVideo *trtc_handle_ptr_;

    QDateTime lastTime;
    QTimer *timerCheck;             //定时器检查设备是否在线
    QImage image;                   //要显示的图片
//    TurningLight *flowPanel0;             //悬浮条面板
//    SpeedLimit *flowPanel1;             //悬浮条面板
//    VehicleMode *flowPanel2;             //悬浮条面板
//    VehicleSpeed *flowPanel3;             //悬浮条面板
//    VehicleGear *flowPanel4;             //悬浮条面板
//    VehicleSteer *flowPanel5;             //悬浮条面板
//    ErrorCode *flowPanel6;             //悬浮条面板
//    VehicleBoard *flowPanel7;        //悬浮条面板
    SignalRssi *flowPanel8;          //悬浮条面板
//    VehicleState *flowPanel9;
//    MessageBox *flowPanel10;
//    MessageBar *flowPanel11;
...
public slots:
    ...
    void initFlowStyle();
    //接收图像并绘制
    void updateImage(double timestamp, const QImage &image);
    //校验设备
    void checkVideo();
    //处理按钮单击
    void btnClicked();
...
public slots:
    // 更新信号灯状态
    void seTurningLightState(int state);
    // 设置限速值
